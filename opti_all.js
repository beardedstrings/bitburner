...
Date: 7/2022
Description: This script is used to optimize host resources (threads/RAM) in a continuous weaken, grow, and hack cycle.
All logs are disabled and a tail is used to monitor the script. This should typically be deployed on a brain/manager/server.
Typical the 'home' device. The script will run on the 'home' device and will automatically deploy tools and start running.
You can either run the scan to attack all servers rooted, or manually change the servers = [] array. Host servers with lower
specs should focus on a limited amount of targets until skills and/or compute resources are available.

Example usage: run optimize_all.js home
run optimize_all.js workerServerName
*/
...
export async function main(ns) {
	...
	while (true) {
		...
		servers.push("n00dles");
		servers.push("foodnstuff");
